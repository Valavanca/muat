...
class TCGA_PCAWG_EPI_Dataloader(Dataset):
    ...
    def __init__(
        ...
        #load config training-val split
        #self.old_config()
        self.training_fold = pd.read_csv(self.args.trainsplit_file,index_col=0)
        self.training_fold = self.training_fold.loc[self.training_fold['fold'] == self.curr_fold]
        self.training_fold = self.training_fold.reset_index(drop=True)

        self.validation_fold = pd.read_csv(self.args.valsplit_file,index_col=0)
        self.validation_fold = self.validation_fold.loc[self.validation_fold['fold'] == self.curr_fold]
        self.validation_fold = self.validation_fold.reset_index(drop=True)
        ...
        self.pd_epi_vocab = pd.read_csv(self.cwd + 'extfile/epigen_token.csv',index_col=0)
        ...
        self.epi_size =  len(self.pd_epi_vocab)
        ...
        elif self.grabdata == 2:
            self.midstring = '.' + self.dataset_name + str(mutratio) + str(int(self.getmotif)) + str(int(self.getposition)) +  str(int(self.getges)) +  str(int(self.getepi)) + '/' 
    ...
    def old_config(self):
        ...
        if self.single_pred_vcf:
            ...
            pd_count = pd.read_csv(args.tmp_dir + 'count_new_' + input_filename[:-4] + '.csv', index_col=0)['0'].to_list()
    ...
    def load_classinfo(self):
        pd_data = pd.read_csv(self.args.classinfo_file,index_col = 0)
    ...
    def get_data(self,idx):
        ...
        if self.grabdata == 2:
            ...
            grabcol=[]
            if self.getmotif:
                grabcol.append('triplettoken')
            if self.getposition:
                grabcol.append('postoken')
            if self.getges:
                grabcol.append('gestoken')
            if self.getepi:
                epicolumns = pd_nisi[pd_nisi.columns.difference(['triplettoken','postoken','gestoken','rt'])].columns.tolist()
                grabcol = grabcol + epicolumns

            #pdb.set_trace()

            #combine all data
            if self.mode == 'training':
                pd_nisi = allgrab(grabcol)
            else:
        ...
        if len(pd_nisi) < self.block_size:
            mins = self.block_size - len(pd_nisi)
            is_padding = True
        ...
        for i in pd_nisi.columns:
            if i in ['triplettoken','postoken','gestoken','rt']:
                ...
                np_data = pd_nisi[epicolumns].to_numpy() 
                #create padding
                if is_padding:
                    pd_nisi_shape = pd_nisi[epicolumns].shape
                    pad_zeros = np.zeros((self.block_size - pd_nisi_shape[0], pd_nisi_shape[1]),dtype=int)
                    np_data = np.concatenate((np_data, pad_zeros), axis=0)
        ...
        datastring = [samples,target_name]
...
if __name__ == '__main__':
    ...
    dataloaderVal = TCGA_PCAWG_Dataloader(dataset_name = 'wgsgx', 
