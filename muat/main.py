...
def execute_annotation(args, only_input_filename):
    ...
    input_file = tmp_dir / f'{only_input_filename}.tsv.gz'
    output_gc_file = tmp_dir / f'{only_input_filename}.gc.tsv.gz'

    # GC content annotation
    annotate_mutations_with_gc_content(
        input_file=str(input_file),
        output_file=str(output_gc_file),
        window=1001,
        label='gc1kb',
    ...
    tmp_dir = Path(args.tmp_dir)
    genomic_tracks_dir = Path(args.genomic_tracks)

    # Genic regions annotation
    input_gc_file = tmp_dir / f'{only_input_filename}.gc.tsv.gz'
    genic_bed_file = genomic_tracks_dir / 'Homo_sapiens.GRCh37.87.genic.genomic.bed.gz'
    output_genic_file = tmp_dir / f'{only_input_filename}.gc.genic.tsv.gz'
    annotate_mutations_with_bed(input_gc_file, genic_bed_file, output_genic_file, label='genic')
    input_gc_file.unlink()

    # Exon regions annotation
    input_genic_file = output_genic_file
    exonic_bed_file = genomic_tracks_dir / 'Homo_sapiens.GRCh37.87.exons.genomic.bed.gz'
    output_exonic_file = tmp_dir / f'{only_input_filename}.gc.genic.exonic.tsv.gz'
    annotate_mutations_with_bed(input_genic_file, exonic_bed_file, output_exonic_file, label='exonic')
    input_genic_file.unlink()
...
if __name__ == '__main__':
        ...
        if args.predict:
            ...
            ckpt_path = args.load_ckpt_dir + args.load_ckpt_filename
            allckpt = torch.load(ckpt_path, map_location=device)
            ...
            try:
                mconf = ModelConfig(
                    vocab_size=validation_dataset.vocab_size,
                ...
                #solving
                args = solving_arch(args) #Workaround for solving arch
