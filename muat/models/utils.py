...
from models.model import (
    ...
    EmbFC,
    MuAtMotifPosition,
    MuAtMotifPositionGES,
    MuAtMotifPositionGESRT,
    MuAtMotifPositionF,
    MuAtMotifPositionGESF,
    MuAtMotifF,
    EmbFCPos,
    EmbFCPosGES,
    DNN_GX,
    MuAtEpiF,
    MuAtMotifPositionGESEpiF,
    MuAtMotifPositionGESEpiF_OneEpiEmb,
    MuAtMotifPositionGESEpiF_EpiCompress,
    MuAtEpiF_EpiCompress
)
from dataset.pcawg_tcga_dataloader import TCGA_PCAWG_Dataloader
from dataset.pcawg_tcga_epi_dataloader import TCGA_PCAWG_EPI_Dataloader
...
def get_simplified_dataloader(args,train_val,input_filename):
    if args.dataloader == 'pcawg' or args.dataloader == 'wgspcawg':
        if args.multi_pred_vcf:
            dataloader_class = TCGA_PCAWG_Dataloader(dataset_name = args.dataloader, 
    ...
    if args.dataloader == 'wgsgx':
        if args.train:
            #pdb.set_trace()
            dataloader_class = TCGA_PCAWG_Dataloader(
    ...
    if args.dataloader == 'tcga-mpg':
        if args.train:
            dataloader_class = TCGA_PCAWG_EPI_Dataloader(
                                dataset_name = args.dataloader, 
                                data_dir=args.input_data_dir, 
                                mode=train_val, 
                                curr_fold=args.fold, 
